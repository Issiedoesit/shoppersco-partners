...
const OrdersHistory = () => {
    ...
    const [ handleSearch, handleBlur ] = useSearchTables('', 'order-history-row')
  ...
  return (
    <div className='rounded-ten bg-white col-span-4 py-9 px-6'>
       ...
       <div className='pb-3 flex flex-col sm:flex-row sm:items-center gap-5 justify-between w-full'>
            <h1 className='font-avenirBlack whitespace-nowrap w-fit'>Order History</h1>
              <label htmlFor="orderHistorySearch" className='rounded-ten py-2.5 border-0.5 flex flex-row items-center gap-2 pl-2 border-brandGray4x sm:w-sixtyPercent sm:max-w-md'>
                ...
                <input type="search" onChange={handleSearch} onBlur={handleBlur} name="orderHistorySearch" id="orderHistorySearch" placeholder='Search by name. date, location or status' className='placeholder:text-xs w-full focus:outline-none focus:ring-none text-sm'/>
            </label>
        </div>
